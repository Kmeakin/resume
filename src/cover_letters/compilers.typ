...
#coverletter-content[
  ...
  opportunities to work on compilers at Arm.
  ...
  Although my work on Rust was mostly focused on back-end optimisations, I did
  investigate adding a new lint to the Rust front-end to detect ABI
  ...
  optimisations and canonicalisation of commutative operators in the Cranelift
  ...
  C++ codebases (LLVM). As part of my responsibilities in the Trusted Firmware
  team, I have pushed to incorporate more static and dynamic checks into the
  codebase to ensure correctness and reliability. For example, I enabled
  compile-time checking of format strings for `printf` in the Hafnium codebase,
  which required rewriting our `printf` implementation to conform to the C
  standard. I have led the migration of the Hafnium codebase from clang 16 to
  clang 18, and hope to migrate to clang 19 and C23 in the near future.

  = Assembly experience
  I have a strong knowledge of the Arm instruction set, both the 32-bit version
  ...
  and trying to find missed optimisation opportunities. I have filed several
